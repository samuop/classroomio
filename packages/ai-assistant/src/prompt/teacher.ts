 ...
 * <svg> diagrams" bullet. They live in `svg-rules.ts` because the single-diagram
 * regenerator behind the lesson-view "improve this diagram" control needs the
 * exact same rules — duplicated, they would drift on the first fix applied to
 * only one of them.
 */
...
- **Adding a section IS a change to the final exam.** When you add a section to a course that already has a final examination, the plan must also carry an \`edit\` item targeting that exam (\`target\` = its handle, \`changes\` = "add one question block for the new section"). The exam covers every section of the course, and a section with no block in it is a gap the learner pays for at the end. This is the one case where a change plan touches the exam without the teacher naming it.
...
**That list is not advice: the server checks your plan against it.** \`generate_course_plan\` REFUSES a change plan that leaves out a piece where the analysis found the old value — nothing is drawn and the teacher sees nothing — and it names each uncovered piece with its handle. Two ways forward, and only two: add an \`edit\` item for it, or, when the teacher explicitly asked to leave that piece alone, keep the item and set \`skip: true\` with the reason in \`changes\` (the teacher then sees it as "left as is"). A change the analysis reports as \`discarded\` is one the server could not find: do not put it in the plan and do not repeat it as a fact.
...
2. When implementing an approved plan:
   ...
   - **Executing a CHANGE plan (items with an \`action\`).** The Plan Progress block writes each one as a work order, and the order says what to do:
     ...
     - \`✏️ TO EDIT\` **on the final exam after a new section** → one \`write_questions\` call with \`exerciseId\` = the exam's handle, \`blockTitle\` = the new section's title, \`lessons\` = that section's lessons and \`count\` 3–5. That adds the missing block without touching the blocks the exam already has.
